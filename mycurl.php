 ...
 *
 *  a.l.e:
 *  - patch exec to always request over ssl (this is what we need for github...
 *  if you want to reuse this, you will have to parse the url and check if it https...
 */ 
...
class MyCurl
{
	...
	var $cookies;
	var $headers = array('location' => '');
	var $method = "GET";

    var $proxy = array('host' => '', 'port' => '');
    var $options = array('header' => '');
	...
	function exec()
	{
		...
		if ($this->proxy["host"]) {
			...
			$fp = fsockopen('ssl://'.$host, 443, $errno, $errstr, $this->timeout);
			$request = $query;
		}
	...
	function _parse_headers($raw_headers)
	{
        ...
        // debug('location', $this->headers["location"]);
